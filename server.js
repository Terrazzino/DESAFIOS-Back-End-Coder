...
app.use((req, res, next) => {    res.json({ error: -2, descripcion: `ruta ${req.path} método ${req.method} no implementada` })})
...
fs.writeFileSync('./txt/productos.txt',JSON.stringify(productos,null,2))
fs.writeFileSync('./txt/carrito.txt',JSON.stringify(carrito,null,2))
...
routerProductos.post('/',(req,res)=>{
    ...
    ID++
    fs.appendFileSync('./txt/productos.txt',JSON.stringify(productos,null,2))
    // res.json(productos);
    res.render('productos',{prod:productos});
});
...
routerCarrito.post('/',(req,res)=>{
    ...
    fs.appendFileSync('./txt/carrito.txt',JSON.stringify(carrito,null,2))
    carritoID++;
    // res.json(carrito);
    res.render('carrito',{carr:carrito});
